...
const LoyaltyApp = () => {
  ...
  const [deviceMode, setDeviceMode] = useState('DETECTING...'); // To show user what mode they are on
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Give UI a moment to render before processing
  ...
  const processCSV = (text) => {
    ...
    // 1. Detect Mobile (Screen width less than 768px is a standard breakpoint)
    const isMobile = window.innerWidth < 768;
    const modeName = isMobile ? "MOBILE MODE (10k LIMIT)" : "DESKTOP MODE (FULL DATA)";
    ...
    // 2. Set the limit based on device
    // If mobile, stop at 10,000. If desktop, go to the end.
    const maxRows = isMobile ? 10000 : lines.length;

    for (let i = 1; i < lines.length; i++) {
      // 3. Stop processing if we hit the limit
  ...
  const handleSearch = () => {
    ...
    if (customer) {
      ...
      // Improved error message to explain why an ID might be missing on mobile
      if (deviceMode.includes("MOBILE") && id > 0) { // Assuming IDs roughly correlate to order
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b-2 border-gray-200 pb-4 mb-4">
          <div>
            ...
            <div className="flex flex-col md:flex-row md:items-center gap-2 mt-2">
                <p className="font-mono text-xs md:text-sm text-gray-500 tracking-wide">
                ALGORITHM: BST (O(log n)) | RECORDS: <span className="tabular-nums font-bold text-gray-700">{rawData.length.toLocaleString()}</span>
                </p>
                {/* Mode Indicator */}
